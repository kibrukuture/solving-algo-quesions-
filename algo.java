...
public class Main {
    ...
    static int f7(int[] a) {
        ...
        int[] lt;
        int[] rt;
        int pos = -1;//left and right sub array from the point of equilibrium.
...
//1,2,3,4,5,6: n(n+1)
    ...
    static int isNUnique(int[] a, int n) {
        ...
        int[][] pairs = new int[Combination.get(a.length, 2)][2];
        int trace = 0;
        for (int i = 0; i < a.length; i++) {
            for (int j = i; j < a.length; j++) {
                if (j == a.length - 1) {
    ...
    static int isLegalNumber(int[] a, int base) {
        ...
        for (int i = 0; i < a.length; i++) {
            if (a[i] > base || a[i] < 0)
                valid = 0;// digits in array "a" should be between 0(included) and base(excluded).
        }
        return valid;
    }
...
//
    ...
    static int isIsolated2(long n) {
        ...
        long cub = n * n * n;
        int contains = 0;
        long sqrdigit = sqr % 10;
        long cubdigit = cub % 10;
    ...
    static int isTrivalent(int[] a) {
        ...
        if (index == -1 || a.length == 0) return 0;
        boolean isIn = true;
        for (int i = 0; i < a.length; i++) {
            if (!Arr.contains(trielem, a[i])) {
                isIn = false;
...
static int is121Array(int[ ] a){
        class Arr{
            static boolean isBothTipSame (int []a){
                int fromStart=0, fromEnd=0;boolean dCheck=true, uCheck=true;
                for(int i=0;i<a.length;i++){
                    for(int j=i;j<=0;j--){
                        if(a[j]!=1){
                            dCheck=false;
                ...
                return fromStart==fromEnd?true:false;
            }
        }
        System.out.println(Arr.isBothTipSame(a));
        int isValid =0;
        if(a.length<3) return isValid;

        return 0;
}
static int[ ] filterArray(int[ ] a, int n){
        int len=0;int reminder=n;int safeSearch=0;
        while(reminder!=0){
            if(reminder%2!=0) len++;
            reminder=reminder/2;
            safeSearch++;
        }
        if(a.length < safeSearch) return null;
        int [] res=new int[len]; int resIndex=0, aIndex=0;reminder=n;
        while(reminder!=0){
           if(reminder%2!=0) {
...
static int largestAdjacentSum(int[ ] a){
        ...
        for(int i=0;i<a.length;i++){
             ...
             if(i!=a.length-1){
                 prev=a[i]+a[i-1];
                 next=a[i]+a[i+1];
                 if(prev<=next && res<next) res=next;
             }
        }
        return res;
}
static int checkConcatenatedSum(int n, int catlen){
        if(n<0 || catlen<0) return 0;
            ...
            while(rem!=0){
                for(int i=0;i<catlen;i++){
                    ...
                    sum+=rem%10*(tenpowerof);
                }
                rem=rem/10;
                /*
                Without the above long solution I could have used the following short solution;
           ##############################################################################
                sum+= Integer.parseInt(Integer.toString(rem%10).repeat(catlen));
            #################################################################
                but this code uses String and Integer classes. I wanted only to work in primitive data types.
                 */
            }
            return sum==n?1:0;
    ...
    public static void main(String[] args) {
       System.out.print(checkConcatenatedSum(2997,3));
